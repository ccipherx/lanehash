/* Rough speed check of the C port vs the Rust crate: cycles/byte at 4 sizes, L2-resident, RDPRU APERF. */
#include "lanehash.h"
#include <stdio.h>
#include <stdlib.h>
#include <stdint.h>
...
int main(void) {
    ...
    for (size_t s = 0; s < 5; s++) { int n = sizes[s] >= 65536 ? 8 : nb; double c = run(lanehash64, bufs, n, sizes[s]), r = run(lanehash64_c, bufs, n, sizes[s]);
        printf("%6zu B: C port %7.2f cyc/hash (%5.2f B/cyc) | Rust %7.2f cyc/hash (%5.2f B/cyc)\n", sizes[s], c, sizes[s] / c, r, sizes[s] / r); }
    return 0;
}
