...
/// `soft` (T-table) equals `spec` (byte-wise) on random blocks and every byte value at every position.
#[test]
fn ttable_round_equals_spec() {
    ...
    for _ in 0..if cfg!(miri) { 2_000 } else { 65_536 } {
        ...
        assert_eq!(lanehash::aes::soft::aes_round(x, k), lanehash::aes::spec::aes_round(x, k));
    }
    for pos in 0..16 {
        for v in 0..=255u8 {
            let mut x = [0u8; 16];
            x[pos] = v;
            assert_eq!(lanehash::aes::soft::aes_round(x, [0; 16]), lanehash::aes::spec::aes_round(x, [0; 16]), "pos {pos} value {v}");
        }
    }
}
...
fn software_round_equals_aesni() {
    ...
    for _ in 0..if cfg!(miri) { 2_000 } else { 200_000 } {
        ...
        let want = lanehash::aes::spec::aes_round(x, k);
        let got = unsafe {
...
fn software_round_equals_neon() {
    ...
    for _ in 0..if cfg!(miri) { 2_000 } else { 200_000 } {
        ...
        assert_eq!(got, lanehash::aes::spec::aes_round(x, k));
        let mut xk = [0u8; 16];
        for i in 0..16 {
            xk[i] = x[i] ^ k[i];
        }
        assert_eq!(got_absorb, lanehash::aes::spec::aes_round(xk, k));
    }
}
