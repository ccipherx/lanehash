...
H=lanehash,aes,gxhash,rapidhash-v3,xxh3,foldhash,ahash
...
taskset -c 3 $B batch --rounds 5 --align 16 --batch 1,2,4,8 --sizes 128,256,1024,4096 --hashes aes > $O/batch-L2.csv 2>&1
for cfg in "256 64" "1024 16" "4096 4"; do set -- $cfg
  taskset -c 3 $B batch --rounds 5 --align 16 --nbuf $2 --batch 1,2,4,8 --sizes $1 --hashes aes >> $O/batch-L1.csv 2>&1
done
taskset -c 3 $B throughput --rounds 5 --align 16 --hashes lanehash,aes,aes-stream,aes-stream4k --sizes 4096,65536,1048576 > $O/stream.csv 2>&1
taskset -c 3 $B throughput --rounds 3 --align 16 --nbuf 4 --hashes lanehash,aes,gxhash,xxh3,rapidhash-v3,foldhash,ahash --sizes 1048576 > $O/L3-1MiB.csv 2>&1
